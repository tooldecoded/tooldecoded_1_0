...
from toolanalysis.models import ProductImages, Products, Brands
import pandas as pd
...
for i in range(len(df)):
    ...
    if action:
        ...
        sku = get_col(row, 'sku')
        # Image identification: image URL/path
        image = get_col(row, 'image')
        
        if action == 'delete':
            if brand_name and sku and image:
                try:
                    brand_obj = Brands.objects.get(name=brand_name)
                    product = Products.objects.get(sku=sku, brand=brand_obj)
                    product_image = ProductImages.objects.get(product=product, image=image)
                    product_image.delete()
                    print(f"ProductImage {product.name} - {image} deleted")
                ...
                print(f"Row {i+1}: Skipping delete - missing required fields (brand, sku, or image)")
        elif action == 'purge':
            # Purge: Remove all ProductImages records for this product
            if brand_name and sku:
                try:
                    brand_obj = Brands.objects.get(name=brand_name)
                    product = Products.objects.get(sku=sku, brand=brand_obj)
                    
                    # Get count of ProductImages before purging
                    productimages_count = ProductImages.objects.filter(product=product).count()
                    
                    # Delete all ProductImages records for this product
                    ProductImages.objects.filter(product=product).delete()
                    
                    print(f"Purged product {product.name}: Removed {productimages_count} ProductImages records")
                except Brands.DoesNotExist:
                    print(f"Row {i+1}: Brand '{brand_name}' not found")
                except Products.DoesNotExist:
                    print(f"Row {i+1}: Product with SKU '{sku}' and brand '{brand_name}' not found")
            ...
            if brand_name and sku and image:
                try:
                    ...
                    try:
                        product_image = ProductImages.objects.get(product=product, image=image)
                        # Update existing ProductImage (no fields to update, just confirm it exists)
                        product_image.save()
                        print(f"ProductImage {product.name} - {image} exists (no fields to update)")
                    except ProductImages.DoesNotExist:
                        # ProductImage doesn't exist, create it
                        ProductImages.objects.create(product=product, image=image)
                        print(f"ProductImage {product.name} - {image} created (was update action, but didn't exist)")
                except Brands.DoesNotExist:
                ...
                print(f"Row {i+1}: Skipping update - missing required fields (brand, sku, or image)")
        elif action == 'create':
            if brand_name and sku and image:
                try:
                    brand_obj = Brands.objects.get(name=brand_name)
                    product = Products.objects.get(sku=sku, brand=brand_obj)
                    try:
                        # ProductImage exists, don't create duplicate (unique_together prevents this anyway)
                        product_image = ProductImages.objects.get(product=product, image=image)
                        print(f"ProductImage {product.name} - {image} already exists, skipping create")
                    except ProductImages.DoesNotExist:
                        # ProductImage doesn't exist, create it
                        ProductImages.objects.create(product=product, image=image)
                        print(f"ProductImage {product.name} - {image} created")
                except Brands.DoesNotExist:
                ...
                print(f"Row {i+1}: Skipping create - missing required fields (brand, sku, or image)")
        else:
