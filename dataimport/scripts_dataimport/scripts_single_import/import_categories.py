...
from toolanalysis.models import Categories
import pandas as pd
...
for i in range(len(df)):
    ...
    if action:
        ...
        if action == 'delete':
            if fullname:
                try:
                    category = Categories.objects.get(fullname=fullname)
                    category.delete()
                    print(f"Category {fullname} deleted")
                except Categories.DoesNotExist:
                    print(f"Row {i+1}: Category '{fullname}' not found")
            else:
            ...
            if fullname:
                try:
                    ...
                    category.save()
                    print(f"Category {fullname} updated")
                except Categories.DoesNotExist:
                    print(f"Row {i+1}: Category '{fullname}' not found for update")
            else:
            ...
            if fullname:
                try:
                    # Check if exists
                    category = Categories.objects.get(fullname=fullname)
                    print(f"Category {fullname} already exists, skipping create")
                except Categories.DoesNotExist:
                    # Create it
                    ...
                    print(f"Category {fullname} created")
            else:
