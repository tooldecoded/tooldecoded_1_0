    ...
    if action:
        ...
        if action == 'delete':
            if value is not None:
                try:
                    batteryvoltage = BatteryVoltages.objects.get(value=int(value))
                    batteryvoltage.delete()
                    print(f"BatteryVoltage {value} deleted")
                except BatteryVoltages.DoesNotExist:
                    print(f"Row {i+1}: BatteryVoltage '{value}' not found")
                except (ValueError, TypeError):
                    print(f"Row {i+1}: Invalid value '{value}', must be an integer")
            else:
                print(f"Row {i+1}: Skipping delete - missing required field (value)")
        elif action == 'update':
            if value is not None:
                try:
                    batteryvoltage = BatteryVoltages.objects.get(value=int(value))
                    # BatteryVoltages only has value field, nothing to update
                    print(f"BatteryVoltage {value} exists (no fields to update)")
                except BatteryVoltages.DoesNotExist:
                    print(f"Row {i+1}: BatteryVoltage '{value}' not found for update")
                except (ValueError, TypeError):
                    print(f"Row {i+1}: Invalid value '{value}', must be an integer")
            else:
                print(f"Row {i+1}: Skipping update - missing required field (value)")
        elif action == 'create':
            if value is not None:
                try:
                    int_value = int(value)
                    # Check if exists
                    try:
                        batteryvoltage = BatteryVoltages.objects.get(value=int_value)
                        print(f"BatteryVoltage {int_value} already exists, skipping create")
                    except BatteryVoltages.DoesNotExist:
                        # Create it
                        BatteryVoltages.objects.create(value=int_value)
                        print(f"BatteryVoltage {int_value} created")
                except (ValueError, TypeError):
                    print(f"Row {i+1}: Invalid value '{value}', must be an integer")
            else:
                print(f"Row {i+1}: Skipping create - missing required field (value)")
        else:
