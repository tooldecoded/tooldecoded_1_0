...
for i in range(len(df)):
    ...
    if action:
        ...
        if action == 'delete':
            if name:
                try:
                    motortype = MotorTypes.objects.get(name=name)
                    motortype.delete()
                    print(f"MotorType {name} deleted")
                except MotorTypes.DoesNotExist:
                    print(f"Row {i+1}: MotorType '{name}' not found")
            else:
            ...
            if name:
                try:
                    ...
                    if sortorder is not None:
                        try:
                            motortype.sortorder = int(sortorder)
                        except (ValueError, TypeError):
                            print(f"Row {i+1}: Invalid sortorder '{sortorder}', skipping")
                    # Note: name is unique, so can't change it in update
                    
                    motortype.save()
                    print(f"MotorType {name} updated")
                except MotorTypes.DoesNotExist:
                    print(f"Row {i+1}: MotorType '{name}' not found for update")
            else:
            ...
            if name:
                try:
                    # Check if exists
                    motortype = MotorTypes.objects.get(name=name)
                    print(f"MotorType {name} already exists, skipping create")
                except MotorTypes.DoesNotExist:
                    # Create it
                    ...
                    print(f"MotorType {name} created")
            else:
