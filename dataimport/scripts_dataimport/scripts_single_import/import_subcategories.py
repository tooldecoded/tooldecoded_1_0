...
from toolanalysis.models import Subcategories, Categories
import pandas as pd
...
for i in range(len(df)):
    ...
    if action:
        ...
        if action == 'delete':
            if fullname:
                try:
                    subcategory = Subcategories.objects.get(fullname=fullname)
                    subcategory.delete()
                    print(f"Subcategory {fullname} deleted")
                except Subcategories.DoesNotExist:
                    print(f"Row {i+1}: Subcategory '{fullname}' not found")
            else:
            ...
            if fullname:
                try:
                    # Check if exists
                    subcategory = Subcategories.objects.get(fullname=fullname)
                    print(f"Subcategory {fullname} already exists, skipping create")
                except Subcategories.DoesNotExist:
                    # Create it
                    ...
                    if sortorder is not None:
                        try:
                            ...
                            print(f"Row {i+1}: Invalid sortorder '{sortorder}', skipping")
                    subcategory = Subcategories.objects.create(**create_data)
                    ...
                    print(f"Subcategory {fullname} created")
            else:
