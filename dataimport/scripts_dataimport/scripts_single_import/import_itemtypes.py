...
from toolanalysis.models import ItemTypes, Categories, Subcategories, Attributes
import pandas as pd
...
for i in range(len(df)):
    ...
    if action:
        ...
        fullname = get_col(row, 'fullname')
        sortorder = get_col(row, 'sortorder')
        category_fullname = get_col(row, 'categoryfullname')
        subcategory_fullname = get_col(row, 'subcategoryfullname')
        ...
        if action == 'delete':
            if fullname:
                try:
                    ...
                    itemtype.delete()
                    print(f"ItemType {fullname} deleted")
                except ItemTypes.DoesNotExist:
                    print(f"Row {i+1}: ItemType '{fullname}' not found")
            else:
                print(f"Row {i+1}: Skipping delete - missing required field (fullname)")
        elif action == 'update':
            if fullname:
                try:
                    itemtype = ItemTypes.objects.get(fullname=fullname)
                    ...
                    # Note: fullname is unique, so can't change it in update
                    
                    # ManyToMany fields - use .add() only, NEVER .clear()
                    if category_fullname:
                        try:
                            category = Categories.objects.get(fullname=category_fullname)
                            itemtype.categories.add(category)  # Add only, preserves existing
                        except Categories.DoesNotExist:
                            print(f"Row {i+1}: Category '{category_fullname}' not found, skipping")
                    if subcategory_fullname:
                        try:
                            subcategory = Subcategories.objects.get(fullname=subcategory_fullname)
                            itemtype.subcategories.add(subcategory)  # Add only, preserves existing
                        except Subcategories.DoesNotExist:
                            print(f"Row {i+1}: Subcategory '{subcategory_fullname}' not found, skipping")
                    if attribute_name:
                        try:
                            attribute = Attributes.objects.get(name=attribute_name)
                            itemtype.attributes.add(attribute)  # Add only, preserves existing
                        except Attributes.DoesNotExist:
                            print(f"Row {i+1}: Attribute '{attribute_name}' not found, skipping")
                    
                    itemtype.save()
                    print(f"ItemType {fullname} updated")
                except ItemTypes.DoesNotExist:
                    print(f"Row {i+1}: ItemType '{fullname}' not found for update")
            else:
                print(f"Row {i+1}: Skipping update - missing required field (fullname)")
        elif action == 'create':
            if fullname:
                try:
                    # Check if exists
                    itemtype = ItemTypes.objects.get(fullname=fullname)
                    print(f"ItemType {fullname} already exists, skipping create")
                except ItemTypes.DoesNotExist:
                    # Create it
                    ...
                    print(f"ItemType {fullname} created")
            else:
                print(f"Row {i+1}: Skipping create - missing required field (fullname)")
        else:
