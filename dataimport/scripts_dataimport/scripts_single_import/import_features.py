...
for i in range(len(df)):
    ...
    if action:
        ...
        if action == 'delete':
            if name:
                try:
                    feature = Features.objects.get(name=name)
                    feature.delete()
                    print(f"Feature {name} deleted")
                except Features.DoesNotExist:
                    print(f"Row {i+1}: Feature '{name}' not found")
            else:
            ...
            if name:
                try:
                    ...
                    if sortorder is not None:
                        try:
                            feature.sortorder = int(sortorder)
                    ...
                    print(f"Row {i+1}: Feature '{name}' not found for update")
            else:
            ...
            if name:
                try:
                    # Check if exists
                    feature = Features.objects.get(name=name)
                    print(f"Feature {name} already exists, skipping create")
                except Features.DoesNotExist:
                    # Create it
