...
batteryplatform = BatteryPlatforms.objects.get(name='20V MAX*')
v18 = BatteryVoltages.objects.get(value=18)

productlines = [['20V MAX* XR','Dewalt 20V MAX* XR',brand , [batteryplatform], [v18]]]


for p in productlines:
    new_item, created = ProductLines.objects.update_or_create (
    name=p[0], description=p[1], brand=p[2])
    if created:
        print(f"Product Line {p[0]} created, {p[3]} added to Product Line {p[0]}, {p[4]} added to Product Line {p[0]}")
    else:
