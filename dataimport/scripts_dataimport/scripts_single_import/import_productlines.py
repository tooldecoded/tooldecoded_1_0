...
from toolanalysis.models import ProductLines, Brands, BatteryPlatforms, BatteryVoltages
import pandas as pd
...
for i in range(len(df)):
    ...
    if action:
        ...
        batteryplatform_name = get_col(row, 'batteryplatform')
        batteryvoltage_value = get_col(row, 'batteryvoltage')
        
        if action == 'delete':
            if name and brand:
                try:
                    brand_obj = Brands.objects.get(name=brand)
                    productline = ProductLines.objects.get(name=name, brand=brand_obj)
                    productline.delete()
                    print(f"ProductLine {name} (brand: {brand}) deleted")
                except Brands.DoesNotExist:
                    print(f"Row {i+1}: Brand '{brand}' not found")
                except ProductLines.DoesNotExist:
                    print(f"Row {i+1}: ProductLine not found")
            else:
                print(f"Row {i+1}: Skipping delete - missing required fields (name or brand)")
        elif action == 'update':
            if name and brand:
                try:
                    brand_obj = Brands.objects.get(name=brand)
                    ...
                    # Note: name and brand are part of unique_together, so can't change them in update
                    
                    # ManyToMany fields - use .add() only, NEVER .clear()
                    if batteryplatform_name:
                        try:
                            batteryplatform = BatteryPlatforms.objects.get(name=batteryplatform_name)
                            productline.batteryplatform.add(batteryplatform)  # Add only, preserves existing
                        except BatteryPlatforms.DoesNotExist:
                            print(f"Row {i+1}: BatteryPlatform '{batteryplatform_name}' not found, skipping")
                    if batteryvoltage_value is not None:
                        try:
                            batteryvoltage = BatteryVoltages.objects.get(value=int(batteryvoltage_value))
                            productline.batteryvoltage.add(batteryvoltage)  # Add only, preserves existing
                        except BatteryVoltages.DoesNotExist:
                            print(f"Row {i+1}: BatteryVoltage '{batteryvoltage_value}' not found, skipping")
                        except (ValueError, TypeError):
                            print(f"Row {i+1}: Invalid batteryvoltage '{batteryvoltage_value}', must be an integer")
                    
                    productline.save()
                    print(f"ProductLine {name} (brand: {brand}) updated")
                except Brands.DoesNotExist:
                    print(f"Row {i+1}: Brand '{brand}' not found, skipping")
                except ProductLines.DoesNotExist:
                    print(f"Row {i+1}: ProductLine not found for update")
            else:
                print(f"Row {i+1}: Skipping update - missing required fields (name or brand)")
        elif action == 'create':
            if name and brand:
                try:
                    brand_obj = Brands.objects.get(name=brand)
                    try:
                        # ProductLine exists, update it instead
                        productline = ProductLines.objects.get(name=name, brand=brand_obj)
                        ...
                        productline.save()
                        print(f"ProductLine {name} (brand: {brand}) updated (was create action, but productline already existed)")
                        
                    except ProductLines.DoesNotExist:
                        # ProductLine doesn't exist, create it
                        create_data = {
                            'name': name,
                            'brand': brand_obj
                        }
                        ...
                        productline = ProductLines.objects.create(**create_data)
                        
                        # Set ManyToMany fields after creation
                        if batteryplatform_name:
                            try:
                                batteryplatform = BatteryPlatforms.objects.get(name=batteryplatform_name)
                                productline.batteryplatform.add(batteryplatform)
                            except BatteryPlatforms.DoesNotExist:
                                print(f"Row {i+1}: BatteryPlatform '{batteryplatform_name}' not found, skipping")
                        if batteryvoltage_value is not None:
                            try:
                                batteryvoltage = BatteryVoltages.objects.get(value=int(batteryvoltage_value))
                                productline.batteryvoltage.add(batteryvoltage)
                            except BatteryVoltages.DoesNotExist:
                                print(f"Row {i+1}: BatteryVoltage '{batteryvoltage_value}' not found, skipping")
                            except (ValueError, TypeError):
                                print(f"Row {i+1}: Invalid batteryvoltage '{batteryvoltage_value}', must be an integer")
                        
                        print(f"ProductLine {name} (brand: {brand}) created")
                except Brands.DoesNotExist:
