...
from toolanalysis.models import BatteryPlatforms, Brands, BatteryVoltages
import pandas as pd
...
for i in range(len(df)):
    ...
    if action:
        ...
        voltage_value = get_col(row, 'voltage')
        
        if action == 'delete':
            if name:
                try:
                    batteryplatform = BatteryPlatforms.objects.get(name=name)
                    batteryplatform.delete()
                    print(f"BatteryPlatform {name} deleted")
                except BatteryPlatforms.DoesNotExist:
                    print(f"Row {i+1}: BatteryPlatform '{name}' not found")
            else:
            ...
            if name:
                try:
                    ...
                    if brand:
                        try:
                            batteryplatform.brand = Brands.objects.get(name=brand)
                        except Brands.DoesNotExist:
                            print(f"Row {i+1}: Brand '{brand}' not found, skipping")
                    elif brand is None:
                        batteryplatform.brand = None
                    
                    # ManyToMany field - use .add() only, NEVER .clear()
                    if voltage_value is not None:
                        try:
                            voltage = BatteryVoltages.objects.get(value=int(voltage_value))
                            batteryplatform.voltage.add(voltage)  # Add only, preserves existing
                        except BatteryVoltages.DoesNotExist:
                            print(f"Row {i+1}: BatteryVoltage '{voltage_value}' not found, skipping")
                        except (ValueError, TypeError):
                            print(f"Row {i+1}: Invalid voltage '{voltage_value}', must be an integer")
                    
                    batteryplatform.save()
                    print(f"BatteryPlatform {name} updated")
                except BatteryPlatforms.DoesNotExist:
                    print(f"Row {i+1}: BatteryPlatform '{name}' not found for update")
            else:
            ...
            if name:
                try:
                    # Check if exists
                    batteryplatform = BatteryPlatforms.objects.get(name=name)
                    print(f"BatteryPlatform {name} already exists, skipping create")
                except BatteryPlatforms.DoesNotExist:
                    # Create it
                    ...
                    batteryplatform = BatteryPlatforms.objects.create(**create_data)
                    
                    # Set ManyToMany fields after creation
                    if voltage_value is not None:
                        try:
                            voltage = BatteryVoltages.objects.get(value=int(voltage_value))
                            batteryplatform.voltage.add(voltage)
                        except BatteryVoltages.DoesNotExist:
                            print(f"Row {i+1}: BatteryVoltage '{voltage_value}' not found, skipping")
                        except (ValueError, TypeError):
                            print(f"Row {i+1}: Invalid voltage '{voltage_value}', must be an integer")
                    
                    print(f"BatteryPlatform {name} created")
            else:
