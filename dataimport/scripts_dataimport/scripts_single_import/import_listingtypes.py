...
from toolanalysis.models import ListingTypes, Retailers
import pandas as pd
...
for i in range(len(df)):
    ...
    if action:
        ...
        retailer = get_col(row, 'retailer')
        
        if action == 'delete':
            if name:
                try:
                    listingtype = ListingTypes.objects.get(name=name)
                    listingtype.delete()
                    print(f"ListingType {name} deleted")
                except ListingTypes.DoesNotExist:
                    print(f"Row {i+1}: ListingType '{name}' not found")
            else:
            ...
            if name:
                try:
                    ...
                    if retailer:
                        try:
                            listingtype.retailer = Retailers.objects.get(name=retailer)
                        except Retailers.DoesNotExist:
                            print(f"Row {i+1}: Retailer '{retailer}' not found, skipping")
                    elif retailer is None:
                        listingtype.retailer = None
                    # Note: name is unique, so can't change it in update
                    
                    listingtype.save()
                    print(f"ListingType {name} updated")
                except ListingTypes.DoesNotExist:
                    print(f"Row {i+1}: ListingType '{name}' not found for update")
            else:
            ...
            if name:
                try:
                    # Check if exists
                    listingtype = ListingTypes.objects.get(name=name)
                    print(f"ListingType {name} already exists, skipping create")
                except ListingTypes.DoesNotExist:
                    # Create it
                    ...
                    if retailer:
                        try:
                            ...
                            print(f"Row {i+1}: Retailer '{retailer}' not found, skipping retailer")
                    ListingTypes.objects.create(**create_data)
                    print(f"ListingType {name} created")
            else:
